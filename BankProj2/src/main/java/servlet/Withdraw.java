...
public class Withdraw extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2-2. 세션에 담겨있는 Account객체들 중에서 계좌번호id(키)를 통해 특정 acc(벨류)를 찾아 새로만든 Account객체에 담아둔다
		HttpSession session = request.getSession();
		Account acc = (Account)session.getAttribute(id);
		
		// 3.출금처리하고 request에 담고 응답 페이지 위임 
		RequestDispatcher dispatcher = request.getRequestDispatcher("accountinfo.jsp");
		if(acc!=null) {
			acc.withdraw(money); // 세션에서 찾아둔 특정계좌에 출금처리 
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		dispatcher = req.getRequestDispatcher("withdraw.jsp");
